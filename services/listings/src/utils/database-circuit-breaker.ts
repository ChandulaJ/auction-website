/**
 * Database Circuit Breaker for Listings Service
 * 
...
class DatabaseCircuitBreaker {
  ...
  private changeState(newState: CircuitState): void {
    if (this.state !== newState) {
      console.log(`[Listings DB Circuit Breaker] State changed from ${this.state} to ${newState}`);
      this.state = newState;
  ...
  async checkDatabaseHealth(): Promise<boolean> {
    try {
      ...
      console.error('[Listings DB Circuit Breaker] Database health check failed:', error);
      return false;
    }
  }
  ...
  async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(`[Listings DB Circuit Breaker] Attempt ${attempt} failed, retrying in ${retryDelay}ms...`);
...
// Create singleton instance for listings service database operations
export const listingsDatabaseCircuitBreaker = new DatabaseCircuitBreaker({
