/**
 * Database Circuit Breaker for Auth Service
 * 
...
class DatabaseCircuitBreaker {
  ...
  private changeState(newState: CircuitState): void {
    if (this.state !== newState) {
      console.log(`[Auth DB Circuit Breaker] State changed from ${this.state} to ${newState}`);
      this.state = newState;
  ...
  async checkDatabaseHealth(): Promise<boolean> {
    try {
      ...
      console.error('[Auth DB Circuit Breaker] Database health check failed:', error);
      return false;
    }
  }
  ...
  async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(`[Auth DB Circuit Breaker] Attempt ${attempt} failed, retrying in ${retryDelay}ms...`);
...
// Create singleton instance for auth service database operations
export const authDatabaseCircuitBreaker = new DatabaseCircuitBreaker({
  name: 'auth-database',
  failureThreshold: 5,
  successThreshold: 3,
